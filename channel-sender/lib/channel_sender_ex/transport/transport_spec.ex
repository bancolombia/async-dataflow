defmodule ChannelSenderEx.Transport.TransportSpec do
    ...
    quote location: :keep do
      ...
      def get_channel_from_qs(req) do
        ...
        case :lists.keyfind(@channel_key, 1, :cowboy_req.parse_qs(req)) do
          {@channel_key, channel} = resp when byte_size(channel) > 10 ->
            TransportSpec.print_socket_info(req, channel)
            resp
      ...
      def lookup_channel_addr(channel_ref) do
        ...
        execute(
          ...
          fn ->
            Logger.error(
              "Transport #{@__option__} unable to start. channel_ref process does not exist yet, ref: #{inspect(channel_ref)}"
            )

            {:error, <<@invalid_channel_code>>}
          end,
          "lookup_channel_#{channel_ref}"
        )
      end
      ...
      def notify_connected(channel, kind) when is_binary(channel) do
        socket_event_bus =
          get_param(:socket_event_bus, ChannelSenderEx.Core.PubSub.SocketEventBus)
  ...
  def print_socket_info(req, channel) do
