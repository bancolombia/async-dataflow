defmodule ChannelBridgeEx.ApplicationConfig do
  ...
  def load_system_env(config) do
    ...
    Application.put_env(:channel_bridge_ex, :cloud_event_app_identifier, {
      String.to_atom(get_in(config, [:bridge, "cloud_event_app_identifier", "strategy"])),
      get_in(config, [:bridge, "cloud_event_app_identifier", "field"])
    })
