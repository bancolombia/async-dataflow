defmodule StreamsApi.Rest.ChannelRequest do
  defstruct ~w[req_headers req_params body token_claims]a

  @moduledoc """
  A new channel request data
  """
  alias StreamsCore.AppClient
  alias StreamsCore.User

  alias StreamsCore.Utils.JsonSearch
  ...
  def extract_channel_alias(request_data) do
    ...
    request_channel_alias =
      StreamsHelperConfig.get([:streams, "request_channel_identifier"], @default_sessionid_search_path)
  ...
  def extract_application(request_data) do
    app_key = StreamsHelperConfig.get([:streams, "request_app_identifier"], @default_app_ref)
  ...
  def extract_user_info(request_data) do
    user_key = StreamsHelperConfig.get([:streams, "request_user_identifier"], @default_user_ref)
