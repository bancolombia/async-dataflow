defmodule StreamsApi.Rest.RestRouterTest do
  use ExUnit.Case, async: true
  use Plug.Test

  alias StreamsApi.Rest.ErrorResponse
  alias StreamsApi.Rest.RestHelper
  alias StreamsApi.Rest.RestRouter

  alias StreamsApi.Rest.AuthPlug.AuthenticationError
  ...
  test "Should not create channel - missing credentials for JwtParseOnly plug" do
    ...
    cfg = %{
      streams: %{
        "channel_authenticator" => %{"auth_module" => Elixir.StreamsRestapiAuth.JwtParseOnlyProvider},
      }
    }
  ...
  test "Should obtain OK response from liveness probe" do
    with_mocks([
      {StreamsRestapiAuth.JwtParseOnlyProvider, [],
