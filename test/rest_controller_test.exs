defmodule RestControllerTest do
  ...
  test "Should create channel on request" do
    ...
    {status, _headers, body} = request(:post, "/ext/channel/create", [{"content-type", "application/json"}], body)
    assert 200 == status
    assert %{"channel_ref" => channel_ref, "channel_secret" => channel_secret} = Jason.decode!(body)
  end
  ...
  test "Should send message on request" do
    ...
    {status, _headers, body} = request(:post, "/ext/channel/deliver_message", [{"content-type", "application/json"}], body)
    assert 202 == status
    assert "Ok" == body
  end
