defmodule ChannelSenderEx.Persistence.RedisChannelPersistence do
  ...
  def delete_message(channel_ref, message_id) do
    Logger.debug(fn -> "Redis: Deleting message [#{channel_ref}:#{message_id}]" end)
    Redix.noreply_command!(:redix_write, ["HDEL", "msgs_" <> channel_ref, message_id])
  rescue
    e ->
      Logger.error(fn ->
        "Redis: Error while deleting message data [#{channel_ref}:#{message_id}]: #{inspect(e)}"
      end)
  ...
  def get_message(message_id, channel_ref) do
    Redix.pipeline(:redix_read, [
      ...
      ["HGET", "msgs_#{channel_ref}", message_id]
    ])
  rescue
    e ->
