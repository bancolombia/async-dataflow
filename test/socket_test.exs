defmodule SocketTest do
  ...
  test "Should send message", %{message: message, ext_message: ext_message} do
    ...
    assert {commands, {"channel1", :connected, {"app1", "user2"}, pending}} = result
  ...
  test "Should not fail when client re-ack message", %{ext_message: %{message_id: message_id}} do
    already_ack_state = {"channel1", :connected, {"app1", "user2"}, _pending = %{}}

    assert {[], {"channel1", :connected, {"app1", "user2"}, %{}}} ==
             Socket.websocket_handle({:text, "Ack::#{message_id}"}, already_ack_state)
