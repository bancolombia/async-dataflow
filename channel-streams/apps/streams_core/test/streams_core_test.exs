defmodule StreamsCoreTest do
  use ExUnit.Case, async: false

  @moduletag :capture_log
  import Mock
  alias StreamsCore.{AppClient, Channel, CloudEvent, User}

  alias StreamsCore.Boundary.{ChannelManager, ChannelRegistry, ChannelSupervisor}
  ...
  test "should start session" do
    ...
    with_mocks([
      ...
      {:ok, {new_channel, _}} = StreamsCore.start_session(Channel.new("a", AppClient.new("b", nil), User.new("c")))
  ...
  test "should re-start session" do
    ...
    with_mocks([
      ...
      {:ok, {other_channel, _}} = StreamsCore.start_session(Channel.new("a", AppClient.new("b", nil), User.new("c")))
  ...
  test "should handle error starting session" do
    ...
    with_mocks([
      ...
      assert {:error, :channel_sender_unknown_error} ==
               StreamsCore.start_session(Channel.new("a2", AppClient.new("b2", nil), User.new("c2")))
    end
  end
  ...
  test "should route message" do
    with_mocks([
      ...
      {:ok, {new_channel, _}} = StreamsCore.start_session(Channel.new("x", AppClient.new("y", nil), User.new("z")))
      ...
      assert :ok == StreamsCore.route_message("x", CloudEvent.new("a", "b", "c", "d", "e", "f", "g", "h"))
    end
  end
  ...
  test "should not route message to un-existent channel" do
    with_mocks([
      ...
      route_result = StreamsCore.route_message("y", CloudEvent.new("a", "b", "c", "d", "e", "f", "g", "h"))
  ...
  test "should close channel/session" do
    with_mocks([
      ...
      close_result = StreamsCore.end_session("z")
  ...
  test "should parse topology configuration default" do
    assert [k8s: [{:strategy, Cluster.Strategy.Gossip}]] == StreamsCore.topologies()
  end
  ...
  test "should parse topology configuration k8s" do
    ...
    with_mocks([
      {StreamsHelperConfig, [], [get: fn _, _ ->
         %{
            "strategy" => "Elixir.Cluster.Strategy.Gossip",
            "config" => %{
              "mode" => ":hostname",
              "kubernetes_service_name" => "streams",
              "some_other_key" => 10
            }
          }
      end]}
      ...
      assert [k8s: [{:strategy, Cluster.Strategy.Gossip},
                {:config, [kubernetes_service_name: "streams", mode: :hostname, some_other_key: 10]}]]
         == StreamsCore.topologies()
    end
  ...
  test "should parse topology with nil configuration" do
    ...
    with_mocks([
      ...
      assert [k8s: [{:strategy, Cluster.Strategy.Gossip}, {:config, []}]] == StreamsCore.topologies()
    end
