defmodule StreamsHelperConfig.ApplicationConfig do
  @moduledoc false
  ...
  @atom_keys [
    [:streams, "channel_authenticator", "auth_module"],
    [:streams, "cloud_event_mutator", "mutator_module"],
    [:streams, "secrets", "provider"]
  ]
  ...
  defp load_system_env(config) do
    Application.put_env(:channel_streams, :config, config)
    config
  end
